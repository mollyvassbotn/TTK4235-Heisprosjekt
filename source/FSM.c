...
void fsm_idle() {
    if(!check_if_order()){
        ...
        if((get_order_floor() < g_current_floor) || ((get_order_floor() <= g_current_floor) &&(g_between_floors == 1))){
...
void fsm_next_order(){
    if(!check_if_order()){
        ...
        for(int i =3; i> get_current_floor(); i--){
            for(int j=0; j<3; j++){
                if(g_orders[4*j+i]){
        ...
        for(int i=0; i<get_current_floor(); i++){
            for(int j=0; j<3; j++){
                if(g_orders[4*j+i]){
                    g_current_order =4*j+i;
                    g_current_state =MOVING_DOWN;
